...
export const handleImageUpload = async (
  ...
  const fileUrl = await StorageService.uploadFile(file, path, (progress) => {
    if (abortSignal?.aborted) {
      // Note: Firebase uploadTask.cancel() could be used here if we had access to it,
      // but for now we just stop reporting progress and the promise will hang or we can reject.
      // In a more robust implementation, we'd pass the task back.
    }
    onProgress?.({ progress });
  });
...
export const handleVideoUpload = async (
  ...
  if (file.size > MAX_VIDEO_SIZE) {
    throw new Error(
      `Video size exceeds maximum allowed (${MAX_VIDEO_SIZE / (1024 * 1024)}MB)`
    );
  }
...
/**
 * Handles general file upload with progress tracking and abort capability
 * @param file The file to upload
 * @param onProgress Optional callback for tracking upload progress
 * @param abortSignal Optional AbortSignal for cancelling the upload
...
export const handleFileUpload = async (
  ...
  // Max file size 20MB
  const MAX_FILE_SIZE_LIMIT = 20 * 1024 * 1024;
  if (file.size > MAX_FILE_SIZE_LIMIT) {
    throw new Error(
      `File size exceeds maximum allowed (${MAX_FILE_SIZE_LIMIT / (1024 * 1024)}MB)`
    );
  }
